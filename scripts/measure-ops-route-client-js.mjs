...
const opsBuildRoot = path.join(repositoryRoot, "apps/ops/.next");
const manifestsRoot = path.join(opsBuildRoot, "server/app");

const clientJavaScriptBudgets = new Map([
  ["/(auth)/forgot-password/page", 181_000],
  ["/(auth)/login/page", 185_000],
  ["/(auth)/reset-password/page", 184_000],
  ["/(auth)/two-factor/page", 176_000],
  ["/(dashboard)/dashboard/page", 396_000],
]);
...
async function measureRoute(route, manifest) {
  ...
  const measuredChunks = await Promise.all(
    chunks.map(async (chunk) => {
      ...
      const file = path.join(opsBuildRoot, relativePath);
      return {
        file: chunk,
...
function assertClientJavaScriptBudgets(routes) {
  ...
  if (failures.length > 0) {
    throw new Error(`Ops client JavaScript budget failed:\n${failures.join("\n")}`);
  }

  console.log(
    `Ops client JavaScript budgets passed (${clientJavaScriptBudgets.size} routes).`,
  );
}
...
async function main() {
  if (process.argv.includes("--help")) {
    console.log(
      "Build ops first. Default output is deterministic route client-JavaScript JSON; --assert-budgets verifies guarded auth/dashboard limits.",
    );
    return;
  }
  ...
  try {
    ...
    if (error?.code === "ENOENT") {
      throw new Error(
        "Ops build artifacts are missing. Run `pnpm build:ops` first.",
      );
    }
    throw error;
  }
  ...
  if (files.length === 0) {
    throw new Error(
      "Ops client-reference manifests are missing. Run `pnpm build:ops` first.",
    );
  }
