...
export function CustomerHeaderNavigation({
  ...
  if (mobile) {
    ...
    return <div className="col-span-2 grid grid-cols-2 gap-2"><Button render={<Link href="/account" prefetch={false} onClick={closeMenu} />} nativeButton={false} variant="outline" className="col-span-2 min-h-12 rounded-[var(--aura-radius)] border-[color:rgb(16_11_6_/_35%)] bg-transparent text-[var(--aura-ink)]">Account overview</Button><Button render={<Link href="/account/orders" prefetch={false} onClick={closeMenu} />} nativeButton={false} variant="outline" className="min-h-12 rounded-[var(--aura-radius)] border-[color:rgb(16_11_6_/_35%)] bg-transparent text-[var(--aura-ink)]">Orders</Button><Button render={<Link href="/account/delivery" prefetch={false} onClick={closeMenu} />} nativeButton={false} variant="outline" className="min-h-12 rounded-[var(--aura-radius)] border-[color:rgb(16_11_6_/_35%)] bg-transparent text-[var(--aura-ink)]">Delivery</Button><Button render={<Link href="/account/settings" prefetch={false} onClick={closeMenu} />} nativeButton={false} variant="outline" className="min-h-12 rounded-[var(--aura-radius)] border-[color:rgb(16_11_6_/_35%)] bg-transparent text-[var(--aura-ink)]">Settings</Button><Button type="button" variant="outline" className="min-h-12 rounded-[var(--aura-radius)] border-[color:rgb(16_11_6_/_35%)] bg-transparent text-[var(--aura-ink)]" onClick={signOut}>Sign out</Button></div>;
  }

  if (!signedIn) {
    return <Link href="/account/sign-in" prefetch={false} aria-label="Sign in to customer account" className="aura-nav-action inline-flex min-h-20 items-center font-display text-[clamp(1.6rem,2vw,2rem)] tracking-[0.01em] focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[var(--aura-ivory)]"><NavWaveLabel label="Account" /></Link>;
  }
  return <details className="group relative min-h-20"><summary aria-label="Open customer account menu" className="aura-nav-action inline-flex min-h-20 cursor-pointer list-none items-center font-display text-[clamp(1.6rem,2vw,2rem)] tracking-[0.01em] focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[var(--aura-ivory)]"><NavWaveLabel label="Account" /></summary><div className="absolute right-0 top-full grid w-56 overflow-hidden rounded-[var(--aura-radius)] border border-[color:rgb(245_228_199_/_55%)] bg-[var(--aura-ink)] p-2 shadow-2xl"><Link href="/account" className="min-h-11 content-center rounded-sm px-3 font-display hover:bg-[var(--aura-ivory)] hover:text-[var(--aura-ink)]">Account overview</Link><Link href="/account/orders" className="min-h-11 content-center rounded-sm px-3 font-display hover:bg-[var(--aura-ivory)] hover:text-[var(--aura-ink)]">Orders</Link><Link href="/account/delivery" className="min-h-11 content-center rounded-sm px-3 font-display hover:bg-[var(--aura-ivory)] hover:text-[var(--aura-ink)]">Delivery details</Link><Link href="/account/settings" className="min-h-11 content-center rounded-sm px-3 font-display hover:bg-[var(--aura-ivory)] hover:text-[var(--aura-ink)]">Settings</Link><button type="button" className="min-h-11 rounded-sm px-3 text-left font-display hover:bg-[var(--aura-ivory)] hover:text-[var(--aura-ink)]" onClick={signOut}>Sign out</button></div></details>;
}
