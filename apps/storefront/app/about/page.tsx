...
export default function AboutPage() {
  return (
    <>
      <section className="relative min-h-[96svh] overflow-hidden bg-[var(--aura-ink)] px-5 pb-20 pt-28 text-[var(--aura-ivory)] sm:px-8 lg:px-10 lg:pt-32">
        <div className="absolute inset-y-0 right-0 w-[62%] opacity-45 lg:w-[48%]">
          <Image src="/images/bottle-detail.webp" alt="Close studio detail of a Perfume Aura bottle" fill priority sizes="(max-width: 1024px) 62vw, 48vw" className="object-cover" />
        ...
        <div className="relative z-10 mx-auto flex min-h-[72svh] max-w-[94rem] flex-col justify-center">
          <p className="text-xs font-semibold uppercase tracking-[0.2em] text-[color:rgb(245_228_199_/_52%)]">It is not about noise. It is about</p>
      ...
      <section className="bg-[var(--aura-ink)] px-3 py-3 text-[var(--aura-ivory)] sm:px-5">
        <div className="mx-auto grid max-w-[94rem] gap-2 lg:grid-cols-3">
          {[
            ["/images/regent-noir-50ml.webp", "Controlled identity"],
            ["/images/azure-tides-50ml.webp", "Real product media"],
            ["/images/petalia-noir-50ml.webp", "Release-gated commerce"],
          ].map(([src, label]) => (
            <article key={src} className="overflow-hidden rounded-[0.65rem] border border-[color:rgb(245_228_199_/_28%)]">
              ...
              <p className="border-t border-[color:rgb(245_228_199_/_28%)] px-5 py-4 text-center font-display text-2xl">{label}</p>
            </article>
          ))}
        </div>
      </section>
      ...
      <section className="grid min-h-[48rem] bg-[var(--aura-ivory)] text-[var(--aura-ink)] lg:grid-cols-[0.85fr_1.15fr]">
        <div className="flex items-center p-7 sm:p-12 lg:p-16">
          <div>
            <p className="max-w-xs text-xs font-semibold uppercase tracking-[0.18em] text-black/48">Product by product, approval by approval, the public collection takes shape.</p>
      ...
      <section className="bg-[var(--aura-ink)] px-5 py-24 text-[var(--aura-ivory)] sm:px-8 lg:py-36">
        <div className="mx-auto grid max-w-[94rem] gap-12 lg:grid-cols-[1fr_1fr] lg:items-start">
          ...
          <div className="space-y-7 pt-4 text-base leading-8 text-[color:rgb(245_228_199_/_62%)]">
            ...
            <p>The storefront is a controlled staging experience. No inspired-reference title, performance claim, ingredient statement, price, review, or policy becomes public without its own approval trail.</p>
