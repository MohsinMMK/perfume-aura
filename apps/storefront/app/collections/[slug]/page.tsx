...
export default async function CollectionPage({ params }: Readonly<{ params: Promise<{ slug: string }> }>) {
  ...
  return <section className="min-h-[80svh] bg-[var(--aura-ink)] px-3 pb-24 pt-28 text-[var(--aura-ivory)] sm:px-5 lg:pb-32 lg:pt-32"><div className="mx-auto max-w-[94rem]"><div className="max-w-5xl"><p className="text-xs font-semibold uppercase tracking-[0.2em] text-[color:rgb(245_228_199_/_48%)]">Collection</p><h1 className="font-display mt-4 text-[clamp(5rem,12vw,12rem)] leading-[0.74]">{collection.title}</h1><p className="mt-7 max-w-2xl text-sm leading-6 text-[color:rgb(245_228_199_/_58%)]">{collection.description}</p></div><div className="aura-product-grid mt-14 grid gap-2">{collection.products.map((product, index) => <ProductCard key={product.id} product={product} priority={index < 3} />)}</div></div></section>;
}
