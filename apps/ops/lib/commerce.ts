...
const catalogPublicationSchema = z.object({
  ...
  season: z.string().trim().max(160).optional(),
  ...
  sillage: z.string().trim().max(120).optional(),
  ...
  if (value.status === "approved" || value.status === "published") {
    for (const [field, fieldValue] of [
      ["scentFamily", value.scentFamily],
      ...
      ["longevityGuidance", value.longevityGuidance],
      ...
      if (!fieldValue?.trim()) {
        context.addIssue({
          code: z.ZodIssueCode.custom,
          path: [field],
          message: "Required before catalog approval.",
        });
      }
    }
  }
  ...
  const saved = await db.transaction(async (transaction) => {
    ...
    const reviewedContent: ReviewedPublicationContent = {
      ...
      contentFields: {
        ...
        audience: optionalText(parsed.data.audience),
        season: optionalText(parsed.data.season),
        concentration: optionalText(parsed.data.concentration),
        ...
        sillage: optionalText(parsed.data.sillage),
    ...
    const values = {
      ...
      audience: reviewedContent.contentFields.audience,
      season: reviewedContent.contentFields.season,
      concentration: reviewedContent.contentFields.concentration,
      ...
      sillage: reviewedContent.contentFields.sillage,
