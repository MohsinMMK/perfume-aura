...
- **COM-ADR-032 / 017 / 029:** all 20 replacement Signature Series names use their exact owner-supplied standalone backend/frontend names. The 45 carried `owner_approved_title_reference` rows have storefront listing titles in the `Inspired by <brand> <reference>` form. That is listing identity only. India-counsel wording, disclaimer, slug/SEO/ads review, and publication approval remain open. The other 49 inspired rows stay unlistable.
- **COM-ADR-016 / 018–021 / 032 mappings:** 45 inspired rows retain owner-approved title-reference strings; 4 retain owner-approved family mappings with exact product/concentration details pending; `Green Creed` remains explicitly unresolved; 44 still need owner input. Exact rows, evidence metadata, and states are recorded in [`REFERENCE.md`](REFERENCE.md#inspired-mapping-register). `main_list:37` retains the approved `212 VIP Men` identity with an explicit evidence gap; `main_list:35` retains Ulta only as labeled strongest-available retailer evidence. All 20 Signature names have mapping not applicable. No internet guess may silently map a replacement name.
