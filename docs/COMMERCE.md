...
| [Mapping register](REFERENCE.md#main-list-mapping-register) | Reviewed main-list identity register |
...
General product state belongs in [Product behavior](COMMERCE.md#product-behavior); deployment
...
|---|---|---|---|
| CAT-001 | Accepted | Preserve supplied source list exactly during normalization. | All three 2026-08-29 replacement-source checksums match the reviewed artifacts; the structured catalog contains sequential main-list rows 1–94 and Signature Series rows 1–20; source names remain unchanged unless a fresh manual source comparison authorizes verifier updates. |
...
| CAT-003 | Blocked | Approve sale data before publication. | Every selected collection-specific product × size row has SKU, integer-paise INR retail price and cost, numeric opening stock, reorder level, and approved launch status; its parent product has approved media. All 322 retail prices are fixed by COM-ADR-032. Parallel size/price list encoding is forbidden. |
| CAT-004 | Accepted | Model each fragrance as one product and each purchasable size as a variant. | All 114 selected products have explicit collection-specific intake rows: main products use 30/50/100 ml and Signature products use 50/105 ml. No 10 ml variant is public commerce. Only fully approved variants may become purchasable. |
...
| OPS-001 | Verified | Deploy the storefront through a separately verified artifact. | GitHub builds a checksummed Linux standalone and immutable GHCR image; the VPS verifies its source and health. Exact live status is in [Current state](CURRENT_STATE.md#live-system). |
...
| COM-ADR-005 | 2026-07-29 | Superseded | Follow COM-ADR-027 for the canonical storefront domain. | The apex is the storefront; separate shop/staging hostnames must remain absent. |
...
| COM-ADR-023 | 2026-08-02 | Accepted | Keep the public storefront as a separate Next.js application sharing Neon, with `app.perfumeaura.com` as the owner-only operations surface. Canonical routing follows COM-ADR-027. | This preserves one inventory source of truth and avoids Shopify or Vercel production dependencies. |
...
| COM-ADR-032 | 2026-08-29 | Accepted | Replace the active catalog with the exact owner-supplied 2026-08-29 sources: 94 main products and 20 Signature products; price main rows 1–16 at ₹600/₹800/₹1,400 for 30/50/100 ml, main rows 17–94 at ₹450/₹650/₹1,200, and Signature rows at their supplied 50/105 ml prices. Carry forward only previously approved inspired mappings that still identify a replacement row, keep all other inspired titles blank, and keep every legal, import, publication, checkout, and release gate closed. | This supersedes only the source count, source sequence, names, and retail-price fields recorded by COM-ADR-013, COM-ADR-019, COM-ADR-024, COM-ADR-029, and COM-ADR-031. It preserves their still-valid title-pattern, legal, packaging, and fail-closed release boundaries. The replacement mapping digest is `c90c911274322b411783f2bbfadd29c8bd62582b573571923df66a86ad2980b3`; SKU, cost, stock, content, media, legal clearance, production import, publication, checkout, and all release flags remain pending. |
| COM-ADR-033 | 2026-08-29 | Accepted | Research the 49 replacement main-list rows without storefront titles; map 34 rows with a defensible or owner-directed potential brand/reference to Inspired, place the remaining 15 generic, brand-only, conflicting, multi-brand, or non-exact names in a temporary `Unknown` collection using their supplied literal names, and expose all 114 products without changing any supplied price. | The owner explicitly requested the research-led split and temporary Unknown label. The resulting catalog is 79 Inspired, 15 Unknown, and 20 Signature. Exact row evidence and reasons are recorded in the B03 reconciliation packet and `REFERENCE.md`. The approved Inspired mapping digest is `e248eed86501021ef5b2719aa1db335173c08c531473f1d9a047a02bbc03e864`; legal clearance, permanent naming for Unknown, bottle/packaging references, production Neon import, publication, checkout, and every release flag remain separate fail-closed gates. |
...
COM-ADR-032 replaces only the catalog source count, sequence, exact names, and
retail-price fields in COM-ADR-013, COM-ADR-019, COM-ADR-024, COM-ADR-029, and
COM-ADR-031. Their title-pattern, legal, packaging, preview-control, and
fail-closed publication boundaries remain accepted.

COM-ADR-033 completes the discovery-listing identity split for all 94 main
rows. It does not supersede COM-ADR-022's bottle/packaging restriction or any
legal, import, publication, checkout, or release gate.

### Outstanding owner/legal inputs

Accepted scope does not make catalog rows sale-ready. These facts remain required:

- **COM-ADR-033 / 017 / 029:** all 20 replacement Signature Series names use their exact owner-supplied standalone backend/frontend names. All 79 `owner_approved_title_reference` rows have storefront listing titles in the `Inspired by <brand> <reference>` form. The 15 temporary Unknown rows preserve their exact supplied names and carry no invented brand/reference. This is listing identity only. India-counsel wording, disclaimer, slug/SEO/ads review, permanent Unknown naming, and publication approval remain open.
- **COM-ADR-016 / 018–021 / 032 / 033 mappings:** 79 main-list rows now have owner-approved title-reference strings and 15 are explicitly `not_applicable_unknown`. Exact rows, evidence metadata, and reasons are recorded in [`REFERENCE.md`](REFERENCE.md#main-list-mapping-register) and the [B03 reconciliation packet](review-packets/2026-08-29-b03-49-name-reconciliation.md). `main_list:37`, `main_list:74`, and `main_list:87` retain explicit evidence gaps; `main_list:35` retains Ulta only as labeled strongest-available retailer evidence. All 20 Signature names have mapping not applicable. No unsupported brand may silently replace an Unknown name.
...
- **COM-ADR-032 variant prices:** all 322 source variants have owner-supplied retail prices. SKU, cost, opening stock, reorder level, media, content, legal approval, import, and publication remain unresolved. No 10 ml row is sellable or publishable.
...
- all three replacement-source artifact digests and the 114-row transcription;
- requirement and decision IDs;
- 114 selected launch products and 322 selected product/size rows whose sale
  data remains fail-closed until SKU, cost, stock, and approval states pass;
- all 322 owner-supplied integer-paise INR retail prices;
...
- [ ] VPS image, entry file, loopback port and root-owned runtime environment verified
...
- Policy pages are unpublished shells. Hardcoded ₹99/₹999 shipping values
  ...
  [Operating policies](#operating-policies). Those
...
[Launch gates](#launch-gates) and [`CURRENT_STATE.md`](CURRENT_STATE.md) pass.
Use a separate authorized release decision for each public capability.

## Launch gates

All gates remain incomplete. No prerequisite completion authorizes opening a
release flag. Record approval references, never privileged advice or secret values.

| Gate | Owner | Required acceptance |
|---|---|---|
| B01 Legal | Owner / India counsel | Exact 114 names, 79 Inspired references, disclaimer wording/placement, SEO/ads/social, separate label/packaging approval, privacy/retention and policies |
| B02 Catalog | Owner | Reviewed SKU, cost, stock, reorder levels, copy, scent facts, media/rights/alt text; deterministic import digest; separate import/publication authorization |
| B03 Operations | Owner / CA / counsel / logistics | GST/entity/HSN, inclusive prices and rounding, invoices/credit notes, fees/refunds, PIN serviceability, delivery estimates, shipping/returns/cancellations, support staffing and wholesale terms |
| B04 Providers | Owner / providers | Production Google OAuth + One Tap fallback; real SMTP delivery; Cashfree KYC, UPI intent/QR, sandbox success/pending/failure/cancel/duplicate/reordered webhook/expiry/refund/reconciliation; 20-minute payment TTL |
| B05 Staff security | Owner | SMTP, owner TOTP and recovery, mandatory 2FA denial checks, then one invited staff journey and direct owner-only action denials |
| B06 Telemetry/jobs | Engineering / owner | Correct Sentry release/readable stack and privacy-safe PostHog events for both apps; same maintenance secret on runtime/Actions; five bounded maintenance streams accepted after B04 |
| B07 Activation | Owner | B01–B06; separately authorize auth, catalog, inquiries, dual-plane checkout; desktop/mobile journeys; separately authorized low-value UPI purchase/refund and zero-drift reconciliation |

Counsel input: [114-name review](review-packets/2026-08-29-b01-114-name-review.csv)
and [legal surfaces](review-packets/2026-08-29-b01-114-name-legal-surface-inventory.md).
Catalog input: [intake workbook](review-packets/2026-08-29-b02-114-product-catalog-intake.xlsx)
and [identity reconciliation](review-packets/2026-08-29-b03-49-name-reconciliation.md).
Tax input: [CA review](review-packets/2026-08-20-ca-gst-commerce-review.md).
These are current input/evidence attachments, not approvals.

Deferred SEO work: confirm public NAP, hours, geo, category, accessibility,
parking and photos; recover the existing Google Business Profile (no duplicate);
submit the seven-URL sitemap in Search Console/Bing and inspect URLs.
Merchant Center and product indexing require the approved sellable catalog and
truthful, accepted checkout/shipping/tax/returns. Submission never guarantees ranking.
...
remain disabled until the active reverse proxy is proven to replace the configured IP header so it cannot be
